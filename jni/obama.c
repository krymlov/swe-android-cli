...
void obama_test_init() {
}

#define JNI_PRINTF(...) {sprintf(SWE_OBAMA_TMP, __VA_ARGS__);strcat(SWE_OBAMA_OUT,SWE_OBAMA_TMP);}
#define JNI_EXIT(code) return code

int obama_test_main(int argc, char *argv[])
{
  ...
  // show input valued
  JNI_PRINTF("Date and time in UT: day=%d mon=%d year=%d decimal hour=%lf\n", iday, imon, iyar, dhour);
  JNI_PRINTF("\tdecimal geographical coordinates lat=%lf, long=%lf\n", dlat, dlon);
  ...
  JNI_PRINTF("Planet\tecl.long.\tecl.lat.\tdist. AU\tspeed deg/day\n");
  for (ipl = SE_SUN; ipl <= SE_TRUE_NODE; ipl++) {
    ...
    JNI_PRINTF("%10.6lf\t%9.6lf\t%9.6lf\t%9.6lf\n", xx[0], xx[1], xx[2], xx[3]);
  }
  // compute ascendant and houses
  iflag = 0;
  iret = swe_houses_ex(jd_ut, iflag, dlat, dlon, ihsy, cusps, ascmc);
  if (iret < 0) {
    JNI_PRINTF("Unknown problem with house calculation, iret=%d\n", iret);
    return ERR;
  }
  JNI_PRINTF("\nAscendant %10.6lf\tMC %10.6lf\n", ascmc[0], ascmc[1]);
  JNI_PRINTF("House system %s\n", swe_house_name(ihsy));
  for (i = 1; i <= 12; i++) {
    JNI_PRINTF("cusp %2d\t%10.6lf\n", i, cusps[i]);
  }
  JNI_PRINTF("\n");
  return OK;
}
